# Adapted from qwen_api.py
# This has support for the Qwen3-VL-Thinking model using Qwen API with thinking capability
...
def generate_response(queries, model, client=None, media_type="image/jpeg", api_key=None, random_baseline=False, task=None, subtask=None, skip_indices=None, on_response_callback=None, seed=42, enable_thinking=True, thinking_budget=2048):
    """
    生成响应函数（支持思考过程）
    ...
    Args:
        ...
        seed: 随机种子（默认为42）
        enable_thinking: 是否启用思考过程（默认为True）
        thinking_budget: 最大推理过程 Token 数（默认为4096）
    ...
    for k in tqdm(iterator):
        ...
        while curr_retries < max_retries and result is None:
            try:
                ...
                # 如果启用思考且有思考内容，可以获取（如果API返回）
                if enable_thinking and hasattr(completion.choices[0].message, 'reasoning_content'):
                    reasoning_content = completion.choices[0].message.reasoning_content
                    # print(f"Reasoning: {reasoning_content}")  # 可选：打印思考过程
                    result=(reasoning_content, result)
            except Exception as e:
                print(f"Error: {e}")
...
if __name__ == "__main__":
    ...
    # api_key = os.getenv("DASHSCOPE_API_KEY")
    # responses = generate_response(queries, model, api_key=api_key, task="test", subtask="test", enable_thinking=True, thinking_budget=4096)
