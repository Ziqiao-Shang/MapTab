...
def generate_max_len_response(queries, model_path, task=None, subtask=None, seed=42, max_len=32768, image_size=336):
    """
    生成响应函数
    ...
    def encode_image_to_base64(path, image_size=512):
        """
        Encode image to base64 after resizing it to a specified square size.
        
        :param path: Path to the image file.
        :param image_size: Desired output image size (width and height).
        ...
        if isinstance(item, list):
            for type_, content in item:
                if type_ == 'system':
                    ...
                    try:
                        base64_image = encode_image_to_base64(content, image_size=image_size)
                        user_content.append({
                            "type": "image_url", 
                            "image_url": {"url": f"data:image/jpeg;base64,{base64_image}"}
                        })
...
def generate_all_results_response(queries, model_path, task=None, subtask=None, seed=42):
    """
    生成响应函数
    ...
    for k in tqdm(iterator):
        ...
        output = llm.chat(messages, sampling_params=sampling_params)
        answer = output[0].outputs[0].text.strip()
        route = extract_between_tags(answer, "route")
        total_time = extract_between_tags(answer, "Total_Time")
        total_price = extract_between_tags(answer, "Total_Price")
        avg_comfort = extract_between_tags(answer, "Average_Comfort Level")
        avg_reliability = extract_between_tags(answer, "Average_Reliability")
...
def generate_parallel_response(queries, model_path, task=None, subtask=None, seed=42):
    """
    生成响应函数
...
def generate_phi4_response(queries, model_path, task=None, subtask=None, seed=42):
    """
    生成响应函数
...
def generate_dif_pic_size_response(queries, model_path, downsampling_rate=1, task=None, subtask=None, seed=42):
    """
    生成响应函数
    ...
    def encode_image_to_base64(path, downsampling_rate=1):
        with Image.open(path) as img:
            img = img.convert("RGB")
            w, h = img.size
            pixels = w * h
            ...
            if downsampling_rate > 1:
                target_pixels = pixels / downsampling_rate
        ...
        if isinstance(item, list):
            for type_, content in item:
                if type_ == 'system':
                    ...
                    try:
                        base64_image = encode_image_to_base64(path=content, downsampling_rate=d_r)
                        user_content.append({
                            "type": "image_url", 
                            "image_url": {"url": f"data:image/jpeg;base64,{base64_image}"}
                        })
